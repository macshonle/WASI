/**
 * WASI Capstone Integration Test - WebAssembly Version
 *
 * This test demonstrates comprehensive WASI API usage and produces
 * deterministic output that can be compared with the native C version.
 *
 * Compile: wasm32-wasip2-clang capstone_wasm.c -o capstone.wasm
 * Run: wasmtime run --dir=. --env=TEST_MODE=wasi capstone.wasm
 */
